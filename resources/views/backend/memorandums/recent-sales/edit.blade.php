                    ...
                    <div class="basics-section">
                        <div class="row">
                            <div class="col-sm-8 clarfix title-col">
                                <h3>Edit Recent Sale Property</h3>
                            </div>
                            <div class="col-sm-12 m-t-30"></div>
                            <div class="col-sm-12">
                                <div class="col-sm-6">
                                    <form action="{{route('recent-sales.update', $recent_sale->id)}}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="name" class="form-control" placeholder="Property Name" value="{{ $recent_sale->name }}" required>
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label class="label-small">Address </label>
                                                <input type="text" name="address" class="form-control" placeholder="Address" value="{{ $recent_sale->address }}">
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label class="label-small">Closing Date <em>*</em></label>
                                                <input type="date" name="closing_date" class="form-control" placeholder="Closing Date" value="{{ $recent_sale->closing_date }}">
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label class="label-small">Year Built In </label>
                                                <input type="text" name="year_built" class="form-control" placeholder="Year Built" value="{{$recent_sale->year_built }}">
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="sale_price" class="form-control currency-mask" placeholder="Sale Price" value="{{ $recent_sale->sale_price }}">
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label class="label-small">Price/SF </label>
                                                <input type="text" name="price_per_sf" class="form-control currency-mask" placeholder="Price Per Square Feet" value="{{ $recent_sale->price_per_sf }}">
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label class="label-small"> CAP Rate </label>
                                                <input type="text" name="cap_rate" class="form-control" placeholder="CAP Rate" value="{{ $recent_sale->cap_rate }}">
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label class="label-small">GRM </label>
                                                <input type="text" name="grm" class="form-control" placeholder="GRM" value="{{ $recent_sale->grm }}">
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                <label class="label-small"> Notes </label>
                                                <textarea name="notes" class="form-control" placeholder="Enter Property Notes" cols="10" rows="10">{{ $recent_sale->notes }}</textarea>
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
                                            <div class="form-group">
                                                ...
                                                <img id="img-1" src="{{Storage::disk('s3')->url('assets/'.$recent_sale->image)}}" alt="Property Image" width="200" height="200" />
                                                <span class="inputs-note">Image must be upload of dimensions 450 * 250 </span>
                                            </div>
                                            <div class="hidden-inputs">
                                                <input type="hidden" name="image" id="hidden-field-1" value="{{$recent_sale->image}}">
                                        ...
                                        <div id="sale-div" style="width: 100%;float: left;">
                                            ...
                                            <div id="content-dup" class="row">
                                                @if($recent_sale->units)
                                                    @php $units = json_decode($recent_sale->units) @endphp
                                                    @foreach($units as $unit)
                                                        @foreach($unit as $type => $value)
                                                            <div id="type_div_{{$loop->parent->iteration}}" class="col-sm-3 form-group">
                                                                <label>Enter Unit Type*</label>
                                                                <input type="text" id="type{{$loop->parent->iteration}}" name="unit_types[]" class="form-control" value="{{$type}}" />
                                                            </div>
                                                            <div id="numb_div_{{$loop->parent->iteration}}" class="col-sm-3 form-group">
                                                                <label>Enter No of Units*</label>
                                                                <input type="text" id="numb{{$loop->parent->iteration}}" name="unit_numbers[]" class="form-control" value="{{$value}}" />
                                                            </div>
                                                        @endforeach
                                                    @endforeach
                                                @else
