                ...
                <div class="form-section">
                    <div class="basics-section">
                        <div class="row">
                            ...
                            <div class="col-sm-12">
                                <div class="col-sm-6">
                                    <form action="{{route('update.description.area-maps',$memorandum->id)}}" method="post"
                                          enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                        <div class="col-sm-12">
                                            <div class="form-group">
                                                <label class="label-small">Area Map Image 1</label>
                                                <button class="btn btn-primary" id="select-image-1" onclick="openModal(this, 1)">Select Image</button>
                                                <img @if(isset($rec->area_map1)) style="display: block;" @else style="display:none;" @endif id="img-1" src="{{isset($rec->area_map1) ? Storage::disk('s3')->url('assets/'.$rec->area_map1) : ''}}" alt="cover page photo" width="200" height="200" />
                                                <span class="inputs-note">Image must be upload of dimensions 835 * 940 </span>
                                            </div>
                                        </div>
                                        <div class="hidden-inputs">
                                            <input type="hidden" name="area_map1" id="hidden-field-1" value="{{isset($rec->area_map1) ? $rec->area_map1 : ''}}">
                                        </div>
                                        <div class="col-sm-12">
                                            <div class="form-group">
                                                <label class="label-small">Area Map Image 2</label>
                                                <button class="btn btn-primary" id="select-image-2" onclick="openModal(this, 2)">Select Image</button>
                                                <img @if(isset($rec->area_map2)) style="display: block;" @else style="display:none;" @endif id="img-2" src="{{isset($rec->area_map2) ? Storage::disk('s3')->url('assets/'.$rec->area_map2) : ''}}" alt="cover page photo" width="200" height="200" />
                                                <span class="inputs-note">Image must be upload of dimensions 835 * 940 </span>
                                            </div>
                                        </div>
                                        <div class="hidden-inputs">
                                            <input type="hidden" name="area_map2" id="hidden-field-2" value="{{isset($rec->area_map2) ? $rec->area_map2 : ''}}">
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <a href="{{ route('load.description.plat-maps', $memorandum->id) }}"
                                ...
                                <div class="col-sm-6">
                                    <div class="image-container">
                                        <img src="{{ asset('memorandums/Page-23.jpg') }}" class="img-responsive">
                                    </div>
                                </div>
                            </div>
                        </div>
